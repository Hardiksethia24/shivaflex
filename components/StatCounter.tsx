...
export function StatCounter({ value, suffix = "", label }: { value: number; suffix?: string; label: string }) {
  ...
  return <div ref={ref}><motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="font-display text-4xl text-brand">{count}{suffix}</motion.p><p className="mt-1 text-sm text-slate-600">{label}</p></div>;
}
